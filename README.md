...
15. A `size_it_cs.py` pop-up window displays a preview of what individually exported objects will look like. The one object (displayed at 3X original size) sampled for preview will generally be the one closest to the bottom in the image of size-selected objects. To help locate it in the image, it's size is provided in the preview window.
