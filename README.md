...
11. The "Export sized objects" button will export each size-selected object segment to its own JPEG file. Each file is proportional in size to the object's size and contains the object's image on a white background. Files are written to the input image's folder and named with a timestamp and the segmented object's index number. The intent is to aid in building a custom training dataset of images for machine-learning applications.
12. If closely spaced objects are being grouped (segmented) together, then reduce the "peak_local_max min distance" slider value. If lager objects have multiple segments, then increase that value. 
