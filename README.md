...
Upon selecting a file at startup you are prompted to apply saved settings, if a previously exported saved_settings.json file exists in the program directory, or to use default settings. Displayed image screen sizes are automatically scaled to proportionally fit your screen. Scaling can be changed later with the Ctrl-Left_arrow and Ctrl-Right_arrow keys (use Shift-Control on macOS).
...
13. The "New input" button can load a new input image. You will be prompted to apply any saved settings or use current settings. The image will be automatically scaled and processed. Importing new images without having to restart the program with default settings is useful when analyzing a series of image files that require similar settings. As at startup, the new image is automatically scaled to fit, but scaling can be changed with keystrokes, as explained in [Usage examples](#Usage-examples).
